...
/// @notice This contract can be used to deploy `OkenV1NftPrivate` contracts.
interface IOkenV1NftFactoryPrivate {
    //--------------------------------- events

    /// @notice Emitted when a `OkenV1NftPrivate` contract is deployed or added to this factory.
    /// @param operator Sender deploying or adding the contract
    /// @param nftContract `OkenV1NftPrivate` contract added
    event NftContractAdded(address indexed operator, address indexed nftContract);

    /// @notice Emitted when a `OkenV1NftPrivate` contract is removed from this factory.
    /// @param operator Sender removing the contract
    /// @param nftContract `OkenV1NftPrivate` contract removed
    event NftContractRemoved(address indexed operator, address indexed nftContract);

    //--------------------------------- factory functions

    /// @notice Deploys a `OkenV1NftPrivate` contract.
    ...
    /// @return Address of the newly deployed `OkenV1NftPrivate` contract
    ...
    /// @notice Removes a `OkenV1NftPrivate` contract from the factory.
