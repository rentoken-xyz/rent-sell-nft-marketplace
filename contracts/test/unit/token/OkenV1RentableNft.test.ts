...
import { OkenV1RentableNft } from "../../../typechain-types";
import {
  NAME,
  SYMBOL,
  RENT_MARKETPLACE,
  SELL_MARKETPLACE,
  PLATFORM_FEE,
} from "../../../deploy/token/rentable-nft";

!developmentChains.includes(network.name)
  ? describe.skip
  : describe("OkenV1RentableNft unit tests", function () {
      let nft: OkenV1RentableNft, nftContract: OkenV1RentableNft;
      ...
      beforeEach(async () => {
        ...
        // get deployment
        await deployments.fixture(["rentableNft"]);
        nftContract = await ethers.getContract("OkenV1RentableNft");
      });
      ...
      describe("mintRentableNft()", () => {
        ...
        it("reverts if fee not paid", async () => {
          ...
          await expect(nft.mintRentableNft(minterAddr, "", { value: value })).to.be.revertedWith(
            error
          );
        });
        it("transfers fees to recipient", async () => {
          ...
          await nft.mintRentableNft(minterAddr, "", { value: platformFee });
        ...
        it("sets uri", async () => {
          ...
          const tokenId = await nft.getTokenCounter();
          await nft.mintRentableNft(minterAddr, expected, { value: platformFee });
          const actual = await nft.tokenURI(tokenId);
          assert.equal(actual, expected);
        });
        it("emits an event", async () => {
          await expect(nft.mintRentableNft(minterAddr, "", { value: platformFee })).to.emit(
            nft,
            "RentableNftMinted"
          );
        });
        ...
        it("can handle large uri", async () => {
          ...
          const tokenId = await nft.getTokenCounter();
          await nft.mintRentableNft(minterAddr, uriExpected, { value: platformFee });
        ...
        it("reverts if minting to zero address", async () => {
          ...
          await expect(
            nft.mintRentableNft(ZERO_ADDRESS, "", { value: platformFee })
      ...
      describe("burn()", () => {
        ...
        it("reverts if sender not approved nor owner", async () => {
          ...
          await expect(nft.burnRentableNft(tokenId)).to.be.revertedWith(error);
        });
        it("burns token if owner", async () => {
          let exists = await nft.getExists(tokenId);
          assert(exists);
          await nft.burnRentableNft(tokenId);
        ...
        it("emits an event", async () => {
          await expect(nft.burnRentableNft(tokenId)).to.emit(nft, "RentableNftBurned");
        });
      });
